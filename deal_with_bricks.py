...
def separate_bricks(img_bgr):
    img = brightness_up(img_bgr)
    img = resize_photo(img)
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    kolor = get_most_dominant_color(img_rgb)
    r = kolor[0]
    g=kolor[1]
    b=kolor[2]
    blue_lower = bb8.array([r-55, g-8, b-10])
    blue_upper = bb8.array([r+30, g+8, b+10])
    ...
    # plt.imshow(res)
    # plt.show()
    s = 0 - cv2.cvtColor(res, cv2.COLOR_RGB2GRAY)
    # plt.imshow(s, 'gray')
    # plt.show()
    ret, thresh = cv2.threshold(s, 254, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    plt.imshow(thresh, 'gray')
    plt.show()

    return thresh
def detect_rotation_and_rotate_separation_bricks(img_bin, img):
...
def get_letters_and_distances_bricks(path):
    img_bgr = cv2.imread(path)
    img_bgr = brightness_up(img_bgr)
    img_bgr = resize_photo(img_bgr)

    img_bin_before_rotation = separate_bricks(img_bgr)

    img_bgr_rotated = detect_rotation_and_rotate_separation_bricks(img_bin_before_rotation, img_bgr)
    img_rgb_rotated = cv2.cvtColor(img_bgr_rotated,cv2.COLOR_BGR2RGB)
    img_bin_after_rotation = separate_bricks(img_bgr_rotated)
